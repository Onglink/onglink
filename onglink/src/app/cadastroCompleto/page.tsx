...
const CadastroCompleto: React.FC = () => {
  ...
  const formikBasico = useFormik<BasicFormValues>({
    initialValues: {
      razaoSocial: '', nomeFantasia:'', cnpj: '', email: '', pessoaResponsavel: '',
      cpf: '', telefone01: '', telefone02: '', causaSocial: '', sobreOng: '',
    },
    validationSchema: Yup.object({
      razaoSocial: Yup.string().required('Razão Social é obrigatória'),
      cnpj: Yup.string().required('CNPJ é obrigatório'),
  ...
  const finalizarCadastro = async (uploadValues: UploadFormValues) => {
    ...
    try {
        ...
        if (!logoUrl) throw new Error("É obrigatório fazer o upload da Logo da ONG.");
        ...
        const dadosFinais = {
            ...
            arquivosLegais: [arquivo1Url, arquivo2Url].filter(url => typeof url === 'string' && url.length > 0),
        ...
        setSubmitError(error.response?.data?.error || error.message || "Falha ao finalizar cadastro. Tente novamente.");
    } finally {
  ...
  const renderForm = () => {
    switch(activeStep) {
      case 0: // Básico
        return (
          <form onSubmit={formikBasico.handleSubmit}>
            ...
            <FormInput formik={formikBasico} name="razaoSocial" label="Razão Social" />
            <FormInput formik={formikBasico} name="nomeFantasia" label="Nome Fantasia" />
            <FormGroup formik={formikBasico} fields={[{ name: "cnpj", label: "CNPJ", width: "w-1/2" }, { name: "email", label: "Email", width: "w-1/2" }]} />
            <FormGroup formik={formikBasico} fields={[{ name: "pessoaResponsavel", label: "Pessoa Responsável", width: "w-1/2" }, { name: "cpf", label: "CPF", width: "w-1/2" }]} />
            <FormGroup formik={formikBasico} fields={[{ name: "telefone01", label: "Telefone 01", width: "w-1/2" }, { name: "telefone02", label: "Telefone 02", width: "w-1/2" }]} />
            <FormRadioGroup formik={formikBasico} name="causaSocial" label="Escolha a causa principal" options={[
                { label: 'Ambiental', value: '1' }, { label: 'Animal', value: '2' }, { label: 'Educação', value: '3' }, { label: 'Saúde', value: '4' }, { label: 'Social', value: '5' },
            ]} />
            <div className="mb-4">
              <label className="block mb-1 font-medium">Conte mais sobre a ONG</label>
              <textarea name="sobreOng" rows={4} value={formikBasico.values.sobreOng} onChange={formikBasico.handleChange} onBlur={formikBasico.handleBlur} className="w-full border-1 rounded p-2" />
              {formikBasico.touched.sobreOng && formikBasico.errors.sobreOng && <div className="text-red-500 text-sm mt-1">{formikBasico.errors.sobreOng}</div>}
            </div>
            <div className='flex justify-end'>
              <button type="submit" className="px-4 py-2 bg-green-500 text-white rounded">Próximo</button>
            </div>
      ...
      case 1: // Endereço
        return (
          <form onSubmit={formikEndereco.handleSubmit}>
            <FormGroup formik={formikEndereco} fields={[{ name: "cep", label:"CEP", placeholder:"00000-000", width:"w-1/2"}]} />
            <button type="button" onClick={() => buscarCep(formikEndereco.values.cep)} className="mb-4 px-4 py-2 bg-blue-500 text-white rounded">Buscar CEP</button>
            <FormGroup formik={formikEndereco} fields={[{ name: "endereco", label: "Endereço", width: "w-full", readonly: true }]} />
            <FormGroup formik={formikEndereco} fields={[{ name: "numero", label: "Número", width: "flex-1" }, { name: "complemento", label: "Complemento", width: "w-full" }]} />
            <FormGroup formik={formikEndereco} fields={[{ name: "bairro", label: "Bairro", width: "w-1/2", readonly: true }, { name: "cidade", label: "Cidade", width: "w-1/2", readonly: true }, { name: "estado", label: "Estado", width: "w-1/10 sm:w-1/5", readonly: true}]} />
            <div className="flex space-x-2 justify-end mt-4 gap-4">
              <button type="button" onClick={prevStep} className="px-4 py-2 bg-gray-400 text-white rounded">Voltar</button>
              <button type="submit" className="px-4 py-2 bg-green-500 text-white rounded">Próximo</button>
            </div>
      ...
      case 2: // Redes Sociais
        return (
          <form onSubmit={formikSocial.handleSubmit}>
            <FormGroup formik={formikSocial} fields={[{ name: "linkedin", label: "LinkedIn", width: "w-1/2" }, { name: "facebook", label: "Facebook", width: "w-1/2" }]} />
            <FormGroup formik={formikSocial} fields={[{ name: "instagram", label: "Instagram", width: "w-1/2" }, { name: "twitter", label: "Twitter (Site)", width: "w-1/2" }]} />
            <div className="flex space-x-2 justify-end mt-4 gap-4">
              <button type="button" onClick={prevStep} className="px-4 py-2 bg-gray-400 text-white rounded">Voltar</button>
              <button type="submit" className="px-4 py-2 bg-green-500 text-white rounded">Próximo</button>
            </div>
      ...
      case 3: // Upload (CORRIGIDO)
        return (
          <form onSubmit={formikUpload.handleSubmit}>
            <FileUpload label="Primeiro Arquivo Legal (PDF)" file={formikUpload.values.arquivo1} setFile={(file) => formikUpload.setFieldValue('arquivo1', file)} accept=".pdf" />
            <FileUpload label="Segundo Arquivo Legal (PDF)" file={formikUpload.values.arquivo2} setFile={(file) => formikUpload.setFieldValue('arquivo2', file)} accept=".pdf" />
            <ImageUpload label="Logo da ONG *" image={imagem} setImage={(file) => { setImagem(file); formikUpload.setFieldValue('imagem', file); }} />
            
            <div className="flex space-x-2 justify-end mt-4 gap-4">
              <button type="button" onClick={prevStep} className="px-4 py-2 bg-gray-400 text-white rounded" disabled={isSubmittingFinal}>Voltar</button>
              
              {/* BOTÃO FINAL CORRIGIDO COM LOADING */}
              <button type="submit" className="px-4 py-2 bg-green-500 text-white rounded d-flex align-items-center" disabled={isSubmittingFinal}>
                  {isSubmittingFinal ? (
                      <>
      ...
      <main className='main1'>
        <div className="mx-auto p-2 w-full flex">
          ...
          <div id='sideMenu_cadComp' className='p-2 mb-2 bg-header rounded-2 text-center w-80'>
            ...
            <div className="flex mb-2 flex-full w-full justify-center sm: flex-wrap flex-column">
              {steps.map((step, index) => (
                <StepButton key={step} label={step} active={activeStep === index} onClick={() => !isSubmittingFinal && setActiveStep(index)} />
              ))}
            </div>
          </div>
