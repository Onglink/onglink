...
import  uploadToCloudinary  from '@/app/services/uploadService';
...
const CadastroCompleto: React.FC = () => {
  ...
  // ... (Formiks Basico, Endereco, Social mantidos iguais) ...

  // ---------------- Formik Upload (MODIFICADO) ----------------
  const formikUpload = useFormik<UploadFormValues>({
    initialValues: { arquivo1: null, arquivo2: null, imagem: null },
    onSubmit: async (values) => {
      // Este é o passo final! Aqui acontece a mágica.
      await finalizarCadastro(values);
    },
  });

  // Função que orquestra tudo no final
  const finalizarCadastro = async (uploadValues: UploadFormValues) => {
    ...
    try {
        // 1. Upload dos Arquivos para o Cloudinary (em paralelo para ser rápido)
        console.log("Iniciando uploads para Cloudinary...");
        const [logoUrl, arquivo1Url, arquivo2Url] = await Promise.all([
            uploadToCloudinary(imagem),            // Usa o state 'imagem' ou uploadValues.imagem
            uploadToCloudinary(uploadValues.arquivo1),
            uploadToCloudinary(uploadValues.arquivo2)
        ]);

        if (!logoUrl) throw new Error("Falha ao fazer upload da Logo.");

        // 2. Montar o Objeto Final para o MongoDB
        // Precisamos bater EXATAMENTE com o seu ongSchema
        const dadosFinais = {
            // --- Dados Básicos ---
            ...
            repLegal: formikBasico.values.pessoaResponsavel, // Schema usa repLegal
            cpf: formikBasico.values.cpf,
            telefone: formikBasico.values.telefone01,        // Schema usa telefone (singular)
            causaSocial: formikBasico.values.causaSocial,
            descricao: formikBasico.values.sobreOng,         // Schema usa descricao
            dataFund: new Date(), // Você não tinha esse campo no form, estou pondo a data atual provisoriamente

            // --- Sub-documentos (Endereco e RedeSocial) ---
            endereco: {
                rua: formikEndereco.values.endereco, // Schema usa rua, form usa endereco
            ...
            redeSocial: {
                linkedin: formikSocial.values.linkedin || 'N/A', // Schema requer, então ponha valor default se vazio
                facebook: formikSocial.values.facebook || 'N/A',
                instagram: formikSocial.values.instagram || 'N/A',
                site: formikSocial.values.twitter || 'N/A'       // Schema não tem twitter, mas tem site. Ajustei aqui.
            },

            // --- URLs do Cloudinary ---
            logo: logoUrl,
            arquivosLegais: [arquivo1Url, arquivo2Url].filter(Boolean), // Remove nulls se algum falhou opcionalmente
            imagens: [logoUrl], // Schema requer 'imagens' (array). Usando a logo provisoriamente.

            // --- Vínculo com Usuário ---
            assignedTo: [usuarioLogado._id] 
        };

        console.log("Enviando para o backend:", dadosFinais);

        // 3. Enviar para o seu Back-end Node.js
        await ongService.cadastrarOng(dadosFinais);

        alert("Parabéns! O cadastro da sua ONG foi enviado para análise.");
        router.push('/feed'); // Ou para uma tela de "Obrigado/Aguardando Aprovação"

    } catch (error: any) {
        console.error("Erro no cadastro completo:", error);
        setSubmitError(error.response?.data?.error || error.message || "Falha ao finalizar cadastro.");
    } finally {
        setIsSubmittingFinal(false);
    }
  };

  // ... resto do componente (renderForm, steps, etc.) ...

  // No case 3 (Upload), atualize o botão para mostrar loading:
  // ...
  // <button type="submit" className="px-4 py-2 bg-green-500 text-white rounded" disabled={isSubmittingFinal}>
  //    {isSubmittingFinal ? <Spinner size="sm" /> : "Finalizar Cadastro"}
  // </button>
  // ...

  return (
      // ... seu JSX wrapper ...
      // Adicione o Alert de erro se quiser:
